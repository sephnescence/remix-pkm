...
const expandStoreySpaceKanbans = async (
  ...
  for (const { 0: match, 1: modelType, index } of spaceKanbans.reverse()) {
    if (
      modelType !== 'epiphany' &&
      modelType !== 'inbox' &&
    ...
    for (const space of storey.spaces.sort((spaceA, spaceB) =>
      ...
      spaceInboxListContent += `
            <div>
              <div class="pb-4 flex gap-2">
                <div>
                  <a class="p-2 bg-indigo-950 hover:bg-violet-900 rounded-md" href="/item/create/nSuiteId/${storey.suite_id}/nStoreyId/${storey.id}/nSpaceId/${space.id}/nModelType/${modelType}">Add</a>
                </div>
      ...
      for (const result of results) {
        spaceInboxListContent += `
              <div class="mb-2">
                <a href="/item/edit/eSuiteId/${storey.suite_id}/eStoreyId/${result.storey_id}/eSpaceId/${result.space_id}/eModelType/${modelType}/eModelId/${result.model_id}/eHistoryId/${result.history_id}">
...
export default expandStoreySpaceKanbans
